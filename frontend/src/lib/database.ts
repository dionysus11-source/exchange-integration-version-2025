...
const initTables = () => {
  ...
  try {
    ...
    // 빌드 중 발생하는 'duplicate column' 오류는 무시
    if (error instanceof Error && 'code' in error && error.code === 'SQLITE_ERROR' && error.message.includes('duplicate column name')) {
      console.warn('마이그레이션 경고 (무시됨):', error.message);
    } else {
      console.error('테이블 마이그레이션 오류:', error);
    }
  }
};
